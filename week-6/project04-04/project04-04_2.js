...
/*    JavaScript 7th Edition
      ...
      Filename: project04-04_2.js
*/

// Global variables
let cashBox = document.getElementById("cash"); // Error Fix: Id value needed ""
let billBox = document.getElementById("bill"); // Error Fix: Id value needed ""
let changeBox = document.getElementById("change"); // Error Fix: Id value needed ""

// Event handlers to be run when the cash or bill value changes
cashBox.addEventListener("change", runTheRegister); // Error Fix: specify TheRegister
billBox.addEventListener("change", runTheRegister); // Error Fix: specify TheRegister
...
function runTheRegister() {
   ...
   // Error Fix: Added try/catch error handler for amount < 0 and display "warning"
   try {
    if(changeValue < 0) {
